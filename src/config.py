...
class Configuration(BaseModel):
    ...
    search_max_results: int = Field(
        ...
        description="Maximum number of search results returned per task",
    )
    max_tokens_per_source: int = Field(
        default=600,
        title="Max Tokens Per Source",
        description="Approximate token budget per source in downstream context",
    )
    max_summary_context_chars: int = Field(
        default=12000,
        title="Max Summary Context Chars",
        description="Hard cap for merged summarization context characters",
    )
    task_parallelism: int = Field(
        default=1,
        title="Task Parallelism",
        description="Number of tasks processed in parallel during streaming runs",
    )
    local_llm: str = Field(
        default="llama3.2",
    ...
    def from_env(cls, overrides: Optional[dict[str, Any]] = None) -> "Configuration":
        ...
        # Load project .env automatically so running from `src/` still picks backend config.
        ...
        env_aliases = {
            ...
            "max_summary_context_chars": os.getenv("MAX_SUMMARY_CONTEXT_CHARS"),
            "task_parallelism": os.getenv("TASK_PARALLELISM"),
