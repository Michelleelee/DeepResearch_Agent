...
class DeepResearchAgent:
    ...
    def run_stream(self, topic: str) -> Iterator[dict[str, Any]]:
        ...
        parallelism = max(1, int(self.config.task_parallelism))
        worker_slots = Semaphore(parallelism)
    ...
    def _execute_task(
        ...
        *,
        ...
        has_search_results = bool(search_result and search_result.get("results"))
        ...
        if has_search_results:
            ...
            notice_lines = [f"- {notice}" for notice in notices if notice]

            sources_sections = ["暂无可用来源"]
            if notice_lines:
                sources_sections.append("系统提示：")
                sources_sections.extend(notice_lines)
            sources_summary = "\n".join(sources_sections).strip()

            context_parts = [
                f"任务查询：{task.query}",
                "未检索到可用网页来源。",
                "请基于通用知识给出谨慎总结，并明确说明未引用外部来源，结论可能过时。",
            ]
            if notice_lines:
                context_parts.append("检索错误信息：")
                context_parts.extend(notice_lines)
            context = "\n".join(context_parts)
            ...
            if emit_stream:
                yield {
                    "type": "status",
                    "message": "未检索到可用来源，已降级为无外部来源总结模式",
                    "task_id": task.id,
                    "step": step,
                }
        ...
        try:
            ...
            logger.exception("Task summarization fallback failed: %s", exc)
            summary_text = (
                "未检索到可用外部来源，且模型总结调用失败。"
                "请检查 LLM 网络与 API 配置后重试。"
            )
            if emit_stream:
                yield {
                    "type": "status",
                    "message": f"任务总结降级失败，已使用兜底文案：{exc}",
                    "task_id": task.id,
                    "step": step,
                }
